...
>용어 정리
> - 정점(Vertex) : 위치 개념
> - 간선(Edge) : 위치 간의 관계인 다리
> - 오일러 경로(Eulerian tour) : 모든 경로를 한번만 통과하여 처음 정점으로 되돌아오는 경로.
> - 무방향 그래프(Undirected graph) : 간선에 방향성이 없는 그래프.
> - 방향 그래프(Directed graph) : 간선에 방향성이 있는 그래프.
> - 가중치 그래프[Weighted graph] : 간선에 비용아니 가중치가 할당된 그래프.
> - 차수(Degree) : 무방향그래 하나의 정점에 인접한 정점의 수
> - 진입 차수(In-degree) :
> - 진출 차수(Out-degree) :
